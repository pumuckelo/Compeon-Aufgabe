...
function sendSmsMessage(text, to, from) {
  ...
  //Then we send the messages with the "deliverMessageViaCarrier" function
    ...
    textArray.forEach(() =>
      splittedText.push(textArray.splice(0, 146).join(""))
    );

    //Add suffix to messages
    const messages = splittedText.map(
      (message, i) => message + ` - Part ${i + 1} of ${splittedText.length}`
    );
